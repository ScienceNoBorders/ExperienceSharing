...
-- 1.0.3: OBS 30+/32 兼容（sceneitem get/set_info2）+ 还原 transform 修复
local VERSION = "1.0.3-obs32"
...
-- 解析 OBS 版本号（兼容 "30.2.3" / "31.0.0-mac" 等）
local version = obs.obs_get_version_string() or ""
local ver_major, ver_minor, ver_patch = version:match("(%d+)%.(%d+)%.(%d+)")
if not ver_major then
    ver_major, ver_minor = version:match("(%d+)%.(%d+)")
end
local major = tonumber(ver_major) or 0
local minor = tonumber(ver_minor) or 0
local patch = tonumber(ver_patch) or 0
-- 旧代码用 major 浮点比较（如 major > 29.0）；保留兼容别名
local major_float = major + (minor / 10)

---
-- OBS 30 起 sceneitem transform API 重命名为 *_info2，OBS 32 移除了旧符号。
-- 运行时探测，兼容 OBS 28/29 与 30+。
---@param item any sceneitem
---@param info any obs_transform_info
...
function get_dc_info()
    ...
    elseif ffi.os == "OSX" then
        -- macOS OBS 30+ 使用 screen_capture + display_uuid
        if major > 29 or (major == 29 and minor >= 1) or major_float > 29.0 then
            return {
...
function release_sceneitem()
    ...
    if sceneitem ~= nil then
        ...
        if sceneitem_info_orig ~= nil then
            ...
            -- 原脚本误用 get_info，应 set 才能还原；OBS 30+ 用 set_info2
    ...
    if sceneitem ~= nil then
        ...
        if sceneitem_info.bounds_type == obs.OBS_BOUNDS_NONE then
            ...
            log("WARNING: Found existing non-boundingbox transform. This may cause issues with zooming.\n" ..
                "         Settings have been auto converted to a bounding box scaling transform instead.\n" ..
...
function log_current_settings()
    ...
    log("OBS Version: " .. version .. " (parsed " .. major .. "." .. minor .. "." .. patch .. ")")
    log("Transform API: " .. ((obs.obs_sceneitem_get_info2 ~= nil) and "info2 (OBS 30+)" or "info (legacy)"))
    log("Platform: " .. ffi.os)
...
function script_description()
    return "Zoom the selected display-capture source to focus on the mouse\n" ..
        "v" .. VERSION .. " — compatible with OBS 28–32+ (uses sceneitem_get/set_info2 when available)\n" ..
        "Hotkeys: Settings → Hotkeys → search “Zoom to mouse” / “Follow during zoom”"
end
